...
class Gyroscope:
    ...
    def _update_data(self):
        ...
        while True:
            ...
            pitch = self.twos_complement_hex(buf[12] | buf[13] << 8) / 100
            yaw = self.twos_complement_hex(buf[10] | buf[11] << 8) / 100
            self.data.update({
                "X": self.twos_complement_hex(buf[2] | buf[3] << 8) / 100,
                "Y": self.twos_complement_hex(buf[4] | buf[5] << 8) / 100,
                "Z": self.twos_complement_hex(buf[6] | buf[7] << 8) / 100,

                "yaw_angle_velocity": self.twos_complement_hex(buf[8] | buf[9] << 8) / 100,
                "yaw_angle": yaw,
                "pitch_angle": pitch,
                "towards_direction": self.direction_unit_vector(pitch, yaw),
                "roll_angle": self.twos_complement_hex(buf[14] | buf[15] << 8) / 100,
                "checksum": (buf[16] | buf[17]) << 8
            })
            break

    @staticmethod
    def twos_complement_hex(hex_val):
        bits = 16
        val = hex_val
        if val & (1 << (bits - 1)):
            val -= 1 << bits
